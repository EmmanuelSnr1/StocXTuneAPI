...
import com.stocxtune.api.security.jwt.JwtUtils;
...
public class UserController {
    ...
    private void updateUserInvestments(User user) {
        user.getInvestments().forEach(i -> {
            try {
                this.stockDataRepo.updateStock(i.getAsset());
    ...
    public ResponseEntity<String> sellStock(HttpServletRequest httpServletRequest, @RequestBody Map<String, String> payload) {
        ...
        if (optionalInvestment.isPresent()) {
            ...
            Double sellingPrice = Double.parseDouble(payload.getOrDefault("sellingPrice", investment.getAsset().getLTP().toString()));
    ...
    public ResponseEntity<String> buyStock(HttpServletRequest httpServletRequest, @RequestBody Map<String, String> payload) {
        ...
        if (tempInvestmentId != null) {
            ...
            if (stockSymbol == null)
                return ResponseEntity.badRequest().body("asset symbol/ investment Id required");

            try {
                asset = this.stockDataRepo.getStockFromSymbol(stockSymbol);
                quantity = Integer.parseInt(payload.getOrDefault("quantity", "0"));

                System.out.println("Successfully set: " + asset + " with q: " + quantity);
        ...
        buyPrice = Double.parseDouble(payload.getOrDefault("buyPrice", asset.getLTP().toString()));

        user.buy(asset, quantity, buyPrice);
